...
全科目をまとめた1PDF・採点実感は無いため、予備試験はテキスト変換ではなく
**出典フッター・見出し付きのPDF収集**で対応する（短答=問題＋正答及び配点、
論文=試験問題＋出題の趣旨）。問題・出題の趣旨は、グループ／全科目まとめPDFから
**選択した科目だけを `pdfsplit.js` で切り出す**（科目見出し ［憲法］ 等をPDF.jsで検出。
画像化された一部年度の趣旨は切り出せないため全体を保存）。短答式の正答及び配点は
